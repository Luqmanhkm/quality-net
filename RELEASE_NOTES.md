...
- **BUG-008** (P0): Rails failed to boot entirely whenever `eager_load` was enabled (production environments, and CI) due to a Zeitwerk class-naming mismatch on `AudioWebSocketMiddleware`. Found via this release's own CI gate on the v1.0.0 tag attempt — a strong example of why the gate exists.
